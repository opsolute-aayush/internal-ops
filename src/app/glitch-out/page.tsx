...
// This game's own landing page — unchanged from when it lived at "/",
// just moved here now that "/" is the multi-game mission-select screen
// (see app/page.tsx). Its card there links straight to /register below.
export default function GlitchOutLandingPage() {
  let step = 0;
  ...
  return (
    <main className="flex flex-1 flex-col items-center justify-center px-4 py-16">
      <div className="w-full max-w-xl space-y-8 text-center">
        <div className="fade-slide-in text-left" style={{ animationDelay: delay() }}>
          <Link
            href="/"
        ...
        <div className="fade-slide-in" style={{ animationDelay: delay() }}>
          <GlitchTitle text="GLITCH OUT" className="text-5xl sm:text-6xl" />
        </div>
        <div className="fade-slide-in" style={{ animationDelay: delay() }}>
        ...
        <div className="fade-slide-in" style={{ animationDelay: delay() }}>
          <Link href="/glitch-out/register" className="inline-block w-full sm:w-auto">
            <NeonButton className="w-full sm:w-auto px-10 py-3.5 text-base">Enter Terminal</NeonButton>
          </Link>
        </div>

        <div className="fade-slide-in" style={{ animationDelay: delay() }}>
          <Link href="/glitch-out/admin" className="text-xs uppercase tracking-widest text-neon-100/30 hover:text-cyan-400">
            Game Master Access →
          </Link>
        </div>
      </div>
